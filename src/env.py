...
class HRROenv(gym.Env):
    ...
    def step(self, action):  # sourcery skip: simplify-numeric-comparison
        ...
        if self.process.pressure_supply / 100 >= 120:
            ...
            reward -= (self.process.pressure_supply / 100 - 114)**3

        observation = self._get_obs()

        assert reward >= -1000, print(reward, self.process.__dict__)
    ...
    def end_cycle_step(self, action):
        ...
        if concentrations.mean(axis=0)[0] < self.electrolyte_concentration:
            reward -= 20    # the process aims to meet the initial concentration
        elif concentrations.mean(axis=0)[0] >= self.electrolyte_concentration * 1.2:
            reward -= 20    # avoid exceeding the target concentration by more than 20%
        ...
        if self.process.SEC_Total < 2.5 and self.process.SEC_Total > 0:  # kWh per cycle
            reward += (4 - self.process.SEC_Total)**2
        ...
        if (
            ...
            if self.process.feed_tank_current_volume > 300:
                reward -= (self.process.feed_tank_current_volume - 250)
            else:
                reward += 10

        observation = self._get_obs()

        assert reward <= 16, print(reward, self.process.__dict__)
        assert reward >= -600, print(reward, self.process.__dict__)
